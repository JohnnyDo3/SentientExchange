# ChannelSpecialistAgent Service (x402-enabled)

AI-powered Channel specialist agent - handles customer segmentation and distribution planning for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
```bash
SERVICE_NAME=channel-specialist-agent
SERVICE_DESCRIPTION=AI-powered Channel specialist agent - handles customer segmentation and distribution planning
PORT=3012
...
```bash
# Build image
docker build -f docker/Dockerfile -t channel-specialist-agent:latest .

# Run container
docker run -p 3012:3012 --env-file .env channel-specialist-agent:latest
...
```bash
curl -X POST http://localhost:3012/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Channel specialist agent - handles customer segmentation and distribution planning",
