# FeatureExtractor Service (x402-enabled)

AI-powered Feature extractor - extracts product features from text and websites for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
SERVICE_DESCRIPTION=AI-powered Feature extractor - extracts product features from text and websites
PORT=3007
...
```bash
# Build image
docker build -f docker/Dockerfile -t feature-extractor:latest .

# Run container
docker run -p 3007:3007 --env-file .env feature-extractor:latest
...
```bash
curl -X POST http://localhost:3007/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Feature extractor - extracts product features from text and websites",
