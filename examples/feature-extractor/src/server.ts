...
const app = express();
const featureService = new FeatureExtractorService();
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'feature-extractor',
    version: '1.0.0',
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`# Feature Extractor Metrics
# TYPE requests_total counter
# HELP requests_total Total number of extraction requests
requests_total 0
...
// Main feature extraction endpoint (requires x402 payment)
app.post('/extract', x402Middleware, async (req, res) => {
  ...
  try {
    ...
    // Execute extraction
    const result = await featureService.extractFeatures(validatedData);

    const processingTime = Date.now() - startTime;

    logger.info('Feature extraction completed successfully:', {
      featuresCount: result.productFeatures.length,
...
app.use((req, res) => {
  res.status(404).json({
    ...
    availableEndpoints: [
      'GET /health',
      'GET /metrics',
      'POST /extract (requires x402 payment)',
    ],
  });
});
