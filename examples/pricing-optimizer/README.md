# PricingOptimizer Service (x402-enabled)

AI-powered Pricing optimizer - analyzes competitor pricing and suggests strategy for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
SERVICE_DESCRIPTION=AI-powered Pricing optimizer - analyzes competitor pricing and suggests strategy
PORT=3009
...
```bash
# Build image
docker build -f docker/Dockerfile -t pricing-optimizer:latest .

# Run container
docker run -p 3009:3009 --env-file .env pricing-optimizer:latest
...
```bash
curl -X POST http://localhost:3009/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Pricing optimizer - analyzes competitor pricing and suggests strategy",
