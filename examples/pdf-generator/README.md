# PdfGenerator Service (x402-enabled)

AI-powered PDF generator - converts content to PDF using pdfkit for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
SERVICE_DESCRIPTION=AI-powered PDF generator - converts content to PDF using pdfkit
PORT=3016
...
```bash
# Build image
docker build -f docker/Dockerfile -t pdf-generator:latest .

# Run container
docker run -p 3016:3016 --env-file .env pdf-generator:latest
...
```bash
curl -X POST http://localhost:3016/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "PDF generator - converts content to PDF using pdfkit",
