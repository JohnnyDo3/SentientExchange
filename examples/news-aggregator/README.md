# NewsAggregator Service (x402-enabled)

AI-powered News aggregator - fetches recent AI news from APIs for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
SERVICE_DESCRIPTION=AI-powered News aggregator - fetches recent AI news from APIs
PORT=3005
...
```bash
# Build image
docker build -f docker/Dockerfile -t news-aggregator:latest .

# Run container
docker run -p 3005:3005 --env-file .env news-aggregator:latest
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "News aggregator - fetches recent AI news from APIs",
