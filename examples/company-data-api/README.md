# CompanyDataApi Service (x402-enabled)

AI-powered Company data API - returns funding, pricing, team size from database for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
```bash
SERVICE_NAME=company-data-api
SERVICE_DESCRIPTION=AI-powered Company data API - returns funding, pricing, team size from database
PORT=3004
...
```bash
# Build image
docker build -f docker/Dockerfile -t company-data-api:latest .

# Run container
docker run -p 3004:3004 --env-file .env company-data-api:latest
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Company data API - returns funding, pricing, team size from database",
