# PresentationBuilderAgent Service (x402-enabled)

AI-powered Presentation builder agent - composes final pitch deck from multiple services for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
```bash
SERVICE_NAME=presentation-builder-agent
SERVICE_DESCRIPTION=AI-powered Presentation builder agent - composes final pitch deck from multiple services
PORT=3013
...
```bash
# Build image
docker build -f docker/Dockerfile -t presentation-builder-agent:latest .

# Run container
docker run -p 3013:3013 --env-file .env presentation-builder-agent:latest
...
```bash
curl -X POST http://localhost:3013/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Presentation builder agent - composes final pitch deck from multiple services",
