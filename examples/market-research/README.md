# MarketResearch Service (x402-enabled)

AI-powered Market research - queries market databases for industry data for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
SERVICE_DESCRIPTION=AI-powered Market research - queries market databases for industry data
PORT=3006
...
```bash
# Build image
docker build -f docker/Dockerfile -t market-research:latest .

# Run container
docker run -p 3006:3006 --env-file .env market-research:latest
...
```bash
curl -X POST http://localhost:3006/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Market research - queries market databases for industry data",
