...
import { validateReportRequest } from './utils/validation';

const app = express();
const reportService = new ReportWriterService();
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'report-writer-agent',
    version: '1.0.0',
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`# Report Writer Metrics
# TYPE requests_total counter
# HELP requests_total Total number of report generation requests
requests_total 0
...
// Main report generation endpoint (requires x402 payment)
app.post('/generate', x402Middleware, async (req, res) => {
  const startTime = Date.now();

  try {
    // Validate request
    const validatedData = validateReportRequest(req.body);
    ...
    // Generate report
    const result = await reportService.generateReport(validatedData);

    const processingTime = Date.now() - startTime;

    logger.info('Report generation completed successfully:', {
      sectionsCount: result.sections.length,
      conclusionsCount: result.conclusions.length,
...
export function startServer() {
  return app.listen(PORT, () => {
    logger.info(`🚀 Report Writer Agent service started`);
