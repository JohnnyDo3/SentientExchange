# ReportWriterAgent Service (x402-enabled)

AI-powered Report writer agent - discovers chart generator and summary writer services for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
```bash
SERVICE_NAME=report-writer-agent
SERVICE_DESCRIPTION=AI-powered Report writer agent - discovers chart generator and summary writer services
PORT=3011
...
```bash
# Build image
docker build -f docker/Dockerfile -t report-writer-agent:latest .

# Run container
docker run -p 3011:3011 --env-file .env report-writer-agent:latest
...
```bash
curl -X POST http://localhost:3011/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Report writer agent - discovers chart generator and summary writer services",
