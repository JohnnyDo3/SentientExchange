# ChartGenerator Service (x402-enabled)

AI-powered Chart generator - creates data visualizations using Chart.js for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
```bash
SERVICE_NAME=chart-generator
SERVICE_DESCRIPTION=AI-powered Chart generator - creates data visualizations using Chart.js
PORT=3014
...
```bash
# Build image
docker build -f docker/Dockerfile -t chart-generator:latest .

# Run container
docker run -p 3014:3014 --env-file .env chart-generator:latest
...
```bash
curl -X POST http://localhost:3014/scrape \
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Chart generator - creates data visualizations using Chart.js",
