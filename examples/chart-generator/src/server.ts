...
import { validateChartRequest } from './utils/validation';
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'chart-generator',
    version: '1.0.0',
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`# Chart Generator Metrics
# TYPE requests_total counter
# HELP requests_total Total number of chart generation requests
requests_total 0
...
// Main chart generation endpoint (requires x402 payment)
app.post('/generate', x402Middleware, async (req, res) => {
  const startTime = Date.now();

  try {
    // Validate request
    const validatedData = validateChartRequest(req.body);

    logger.info('Chart generation request received:', {
      type: validatedData.type,
      title: validatedData.title,
      datasetsCount: validatedData.data.datasets.length,
      payment: (req as any).payment?.txHash,
    });

    // Generate chart
    const result = await chartService.generateChart(validatedData);
    ...
    logger.error('Chart generation request failed:', {
      error: error.message,
      processingTime: `${processingTime}ms`,
    });
...
export function startServer() {
  return app.listen(PORT, () => {
    logger.info(`🚀 Chart Generator service started`);
