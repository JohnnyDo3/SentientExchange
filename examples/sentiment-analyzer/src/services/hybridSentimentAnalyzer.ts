...
export class HybridSentimentAnalyzer {
   ...
   */
  async analyze(text: string): Promise<SentimentResult & { method: string; cached: boolean }> {
    ...
    if (this.llmAnalyzer.isAvailable()) {
      try {
        ...
        logger.info('Used LLM analyzer', { confidence: result.overall.confidence });
        return { ...result, method: 'llm', cached: false };
      } catch (error: any) {
        this.stats.errors.llm++;
        logger.warn('LLM analyzer failed, falling back to ML', { error: error.message });
      }
    }
    ...
    try {
      if (await this.mlAnalyzer.isReady()) {
        ...
        logger.info('Used ML analyzer', { confidence: result.overall.confidence });
        return { ...result, method: 'ml', cached: false };
      }
    } catch (error: any) {
      this.stats.errors.ml++;
      logger.warn('ML analyzer failed, falling back to lexicon', { error: error.message });
    }
    ...
    try {
      ...
      logger.info('Used lexicon analyzer', { confidence: result.overall.confidence });
   ...
   * Cache result
   */
  private cacheResult(key: string, result: SentimentResult, method: 'llm' | 'ml' | 'lexicon'): void {
    this.cache.set(key, {
      result,
      timestamp: Date.now(),
      method
  ...
  private cleanCache(): void {
    ...
    if (removed > 0) {
      logger.debug('Cache cleanup', { entriesRemoved: removed, cacheSize: this.cache.size });
    }
  }
  ...
  getStats() {
    ...
    const llmStats = this.llmAnalyzer.isAvailable() ? this.llmAnalyzer.getStats() : null;
    ...
    return {
      ...
      percentages: total > 0 ? {
        ...
        cache: ((this.stats.cache / (total + this.stats.cache)) * 100).toFixed(1) + '%'
      } : null,
