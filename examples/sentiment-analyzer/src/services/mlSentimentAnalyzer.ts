...
export class MLSentimentAnalyzer {
   ...
   */
  private convertToStandardFormat(modelOutput: any, text: string): SentimentResult {
    // Model returns array like: [{ label: 'positive', score: 0.95 }]
    const prediction = Array.isArray(modelOutput) ? modelOutput[0] : modelOutput;
    ...
    const emotions: Array<{ emotion: string; score: number; intensity: 'low' | 'medium' | 'high' }> = [];
    ...
    if (polarity > 0.3) {
      ...
      emotions.push({
        emotion: 'sadness',
        score: Math.min(1.0, Math.abs(polarity)),
        intensity: Math.abs(polarity) > 0.7 ? 'high' : Math.abs(polarity) > 0.5 ? 'medium' : 'low'
      });
    }
    ...
    // Calculate intensity
    const intensity = Math.min(1.0, Math.abs(polarity) + (confidence > 0.9 ? 0.1 : 0));
   ...
   */
  private extractKeywords(text: string, polarity: number): {
    positive: string[];
    negative: string[];
    neutral: string[];
  } {
    ...
    const positiveWords = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic', 'love', 'best', 'awesome', 'perfect', 'happy', 'pleased', 'satisfied', 'helpful'];
    const negativeWords = ['bad', 'terrible', 'awful', 'horrible', 'worst', 'hate', 'disappointed', 'poor', 'frustrating', 'angry', 'sad', 'unhappy'];
  ...
  private estimateSubjectivity(text: string): number {
    ...
    const subjectiveIndicators = ['i think', 'i feel', 'in my opinion', 'i believe', 'personally', 'i', 'me', 'my'];
    subjectiveIndicators.forEach(indicator => {
      if (lowerText.includes(indicator)) subjectiveCount++;
    });

    // Objective indicators
    const objectiveIndicators = ['research', 'data', 'study', 'evidence', 'proven', 'fact'];
