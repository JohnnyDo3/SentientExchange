...
const EMOTION_LEXICON = {
  joy: {
    strong: ['ecstatic', 'thrilled', 'delighted', 'overjoyed', 'elated', 'wonderful', 'amazing', 'fantastic', 'excellent', 'outstanding', 'perfect', 'love', 'fire', 'lit', 'goat', 'legendary', 'epic', 'blessed', 'vibing', 'hyped', 'peak', 'bussin', 'slaps', 'chef', 'immaculate'],
    medium: ['happy', 'pleased', 'glad', 'cheerful', 'content', 'good', 'great', 'nice', 'enjoy', 'satisfied', 'helpful', 'patient', 'reasonable', 'dope', 'cool', 'awesome', 'sick', 'legit', 'solid', 'decent', 'poggers', 'based', 'vibe', 'king', 'queen', 'win'],
    weak: ['ok', 'fine', 'decent', 'alright', 'satisfactory', 'okay', 'mid', 'aight', 'ight', 'bet', 'valid']
  },
  sadness: {
    strong: ['devastated', 'heartbroken', 'miserable', 'depressed', 'anguished', 'awful', 'horrible', 'dreadful', 'crushed', 'destroyed', 'wrecked', 'dead', 'rip'],
    medium: ['sad', 'unhappy', 'disappointed', 'blue', 'melancholy', 'unfortunate', 'sucks', 'bummer', 'rough', 'tough', 'harsh', 'oof', 'pain', 'crying'],
    weak: ['down', 'bummed', 'let down', 'subdued', 'meh', 'bleh', 'yikes']
  },
  anger: {
    strong: ['furious', 'enraged', 'livid', 'outraged', 'incensed', 'pissed', 'heated', 'tilted', 'seething', 'raging'],
    medium: ['angry', 'mad', 'annoyed', 'frustrated', 'irritated', 'salty', 'triggered', 'pressed', 'heated', 'sus'],
    weak: ['bothered', 'irked', 'peeved', 'displeased', 'frustrating', 'bruh', 'smh']
  },
  fear: {
    strong: ['terrified', 'petrified', 'horrified', 'panicked', 'nightmare'],
    medium: ['afraid', 'scared', 'worried', 'anxious', 'nervous', 'sketchy', 'sus', 'creepy', 'spooky'],
    weak: ['uneasy', 'concerned', 'apprehensive', 'uncertain', 'iffy', 'sus']
  },
  surprise: {
    strong: ['astonished', 'astounded', 'shocked', 'stunned', 'shook', 'shooketh', 'wild', 'insane', 'crazy', 'unreal', 'unbelievable'],
    medium: ['surprised', 'amazed', 'startled', 'unexpected', 'exceeded', 'whoa', 'wow', 'damn', 'wtf', 'bruh'],
    weak: ['curious', 'interested', 'intrigued', 'huh', 'wait', 'hmm']
  },
  disgust: {
    strong: ['revolted', 'repulsed', 'sickened', 'disgusted', 'revolting', 'disgusting', 'vile', 'nasty', 'foul', 'trash', 'garbage', 'toxic', 'cringe'],
    medium: ['distasteful', 'unpleasant', 'gross', 'nasty', 'terrible', 'bad', 'awful', 'yuck', 'ew', 'nasty', 'sus', 'mid', 'cap', 'cringe', 'wack', 'lame'],
    weak: ['off-putting', 'disagreeable', 'unappealing', 'boring', 'meh', 'nah', 'nope']
  }
};

// Context modifiers that affect sentiment
const INTENSIFIERS = ['very', 'extremely', 'incredibly', 'absolutely', 'totally', 'really'];
const DIMINISHERS = ['somewhat', 'slightly', 'a bit', 'kind of', 'sort of', 'barely'];
const NEGATIONS = ['not', 'never', "n't", 'no', 'neither', 'nor', 'hardly'];

// Subjective vs objective indicators
const SUBJECTIVE_INDICATORS = ['i think', 'i feel', 'in my opinion', 'personally', 'i believe'];
const OBJECTIVE_INDICATORS = ['research shows', 'data indicates', 'studies suggest', 'evidence', 'proven'];
...
export class SentimentAnalyzer {
  ...
  private detectEmotions(text: string, words: string[]): EmotionScore[] {
    const emotionScores: { [key: string]: { total: number; count: number; maxIntensity: number } } = {};
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      for (const [emotion, intensities] of Object.entries(EMOTION_LEXICON)) {
        ...
        if (matchedIntensity > 0) {
          ...
          if (hasIntensifier) matchedIntensity = Math.min(1.0, matchedIntensity * 1.3);
  ...
  private calculateIntensity(words: string[], emotions: EmotionScore[]): number {
    if (emotions.length === 0) return 0;
  ...
  private detectMixedEmotions(emotions: EmotionScore[]): boolean {
    ...
    const hasPositive = emotions.some(e => positiveEmotions.includes(e.emotion) && e.score > 0.3);
    const hasNegative = emotions.some(e => negativeEmotions.includes(e.emotion) && e.score > 0.3);

    return hasPositive && hasNegative;
  }

  private calculateConfidence(emotions: EmotionScore[], mixed: boolean, wordCount: number): number {
    ...
    const confidence = Math.max(0.1, Math.min(1.0, dominantScore - mixedPenalty + lengthBonus));
    return Math.round(confidence * 100) / 100;
  }

  private extractKeywords(text: string, words: string[]): { positive: string[]; negative: string[]; neutral: string[] } {
    ...
    words.forEach(word => {
      for (const [emotion, intensities] of Object.entries(EMOTION_LEXICON)) {
        const allWords = [...intensities.strong, ...intensities.medium, ...intensities.weak];
    ...
    const commonNeutral = ['thing', 'something', 'anything', 'everything', 'nothing'];
    words.forEach(word => {
      if (commonNeutral.includes(word) && !positive.has(word) && !negative.has(word)) {
        neutral.add(word);
      }
    });
  ...
  private calculateSubjectivity(text: string, words: string[]): number {
    ...
    const firstPersonCount = words.filter(w => ['i', 'me', 'my', 'mine', 'we', 'our', 'us'].includes(w)).length;
  ...
  private categorizePolarity(polarity: number, intensity: number, mixed: boolean): string {
    if (mixed) {
