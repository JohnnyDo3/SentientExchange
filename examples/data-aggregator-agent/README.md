# DataAggregatorAgent Service (x402-enabled)

AI-powered Data aggregator agent - discovers and uses comparison and market share services for competitor analysis. Extracts structured data from websites including pricing, features, contact information, and metadata.
...
```bash
SERVICE_NAME=data-aggregator-agent
SERVICE_DESCRIPTION=AI-powered Data aggregator agent - discovers and uses comparison and market share services
PORT=3010
...
```bash
# Build image
docker build -f docker/Dockerfile -t data-aggregator-agent:latest .

# Run container
docker run -p 3010:3010 --env-file .env data-aggregator-agent:latest
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "/scrape",
    "description": "Data aggregator agent - discovers and uses comparison and market share services",
